<template>
  <div class="max-w-4xl mx-auto p-6">
    <Card>
      <CardHeader>
        <CardTitle>Gönderiyi Düzenle</CardTitle>
        <CardDescription>Aşağıdaki gönderi detaylarını güncelleyin</CardDescription>
      </CardHeader>
      <CardContent>
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Gönderi URL slug'ı"
              class="mt-1"
              readonly
            />
            ...
            <div class="mt-2">
              <RadioGroup v-model="form.status.$value" class="flex space-x-4">
                <div class="flex items-center space-x-2">
                  <RadioGroupItem id="draft" value="draft" :checked="!!form.published_at.$value"/>
                  <Label for="draft">Taslak</Label>
                </div>
                <div class="flex items-center space-x-2">
                  <RadioGroupItem id="publish" value="publish" :checked="!!form.published_at.$value" />
            ...
            >
              ...
              <div v-if="!form.image.$value && !currentImage">
                ...
                <p class="mt-2 text-sm text-gray-600">
                  Buraya bir resim sürükleyip bırakın ya da seçmek için tıklayın
                </p>
  ...
  const resetForm = () => {
    ...
    form.body.$value = initialFormData.body
    form.categories.$value = initialFormData.categories
    form.status.$value = initialFormData.status
    form.published_at.$value = initialFormData.published_at
    form.image.$value = null
  ...
  const fetchPost = async () => {
    try {
      ...
      form.categories.$value = post.categories.map((c: any) => c.id)
      form.status.$value = post.status
      form.published_at.$value = post.published_at || new Date().toISOString().slice(0, 16)
      currentImage.value = post.image_url || null
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(`/api/posts/${postId.value}`, formData, {
        headers: {
  ...
  onMounted(async () => {
    try {
      ...
      categories.value = categoriesRes.data.map((category: any) => ({
        id: category.id,
